...
// Table header
    $pdf->Cell(48, 10, 'Regions', 1, 0, 'C');
    $pdf->Cell(38, 10, 'Complied TT', 1, 0, 'C');
    $pdf->Cell(38, 10, 'Violated TT', 1, 0, 'C');
    ...
    if(mysqli_num_rows($ticketResult)>0){
        ...
        while ($ticket = mysqli_fetch_assoc($ticketResult)) {
            ...
            if ($total_count != 0) {
                ...
                // Round to three decimal places for accuracy
                $avg_min = round($total_min, 3);

                // Convert total minutes to hours and minutes
                $total_seconds = $avg_min * 60;
                $hours = floor($total_seconds / 3600); // Get the total hours
                $minutes = floor(($total_seconds % 3600) / 60); // Get remaining minutes

                // Format the time as 'xh ymins'
